...
class SignedGraphConvolutionalNetwork(torch.nn.Module):
    ...
    def __init__(self, device, args, X):
        ...
        self.X = X
        self.setup_layers()
        # self.criterion = nn.TripletMarginLoss(margin=1, p=2)
    ...
    def calculate_positive_embedding_loss(self, z, positive_edges):
        ...
        # term[term < 0] = 0
        # loss_term = term.mean()
        loss_term = self.criterion(self.positive_z_i, self.positive_z_j, self.positive_z_k)
        return loss_term
    ...
    def calculate_negative_embedding_loss(self, z, negative_edges):
        ...
        loss_term = self.criterion(self.negative_z_i, self.negative_z_k, self.negative_z_j)
        return loss_term
...
class SignedGCNTrainer(object):
    ...
    def score_model(self, epoch):
        ...
        train_positive_z = torch.cat(
            (self.train_z[self.positive_edges[0, :], :], self.train_z[self.positive_edges[1, :], :]), 1)
        train_negative_z = torch.cat(
            (self.train_z[self.negative_edges[0, :], :], self.train_z[self.negative_edges[1, :], :]), 1)

        test = torch.cat((test_positive_z, test_negative_z), 0).detach().cpu().numpy()
        train = torch.cat((train_positive_z, train_negative_z), 0).detach().cpu().numpy()
        train_y = np.array([1]*self.positive_edges.size()[1] + [-1]*self.negative_edges.size()[1])
        test_y = np.array([1]*len(self.test_positive_edges) + [-1]*len(self.test_negative_edges))

        reg_model = LogisticRegression(class_weight='balanced', solver='liblinear')
        reg_model.fit(train, train_y)
        predictions = reg_model.predict(test)
        auc = roc_auc_score(test_y, predictions)
        f1 = f1_score(test_y, predictions)
        # auc, f1 = calculate_auc(test_y, predictions, self.edges)
